        ...
        <div className="flex items-center w-full md:hidden">
          ...
          <button className="focus:outline-none ml-auto mr-4" onClick={toggleMobileMenu}>
            <motion.div animate={isMobileMenuOpen ? "open" : "closed"} variants={iconVariants} transition={{ duration: 0.5 }} style={{
              ...
              <motion.div style={{ position: "absolute" }} initial={{ opacity: 0 }} animate={{ opacity: isMobileMenuOpen ? 1 : 0 }} transition={{ duration: 0.15, delay: isMobileMenuOpen ? 0.25 : 0 }}>
    ...
    <div className="space-x-[2rem]">
        ...
        {isMobileMenuOpen && (
          <>
            ...
            <motion.div
              ...
              exit={{ x: "-100%" }}
              transition={{ duration: 0.3 }}
              className={`fixed z-50 md:hidden ${mobileMenuBg} ${mobileMenuPos} ${mobileMenuWidth} shadow-md`}
      ...
      style={{
        ...
        height: "calc(var(--vh, 1vh) * 100 - 4.6rem)",
            ...
            >
              <div className={`mt-6 flex flex-col px-6 gap-${tailwindData.gapRemValues?.mobile} ${mobileMenuTextAlign}`}>
                {navItems.map((item) => (
                  <NavLink key={item.href} label={item.label} href={item.href} onClick={toggleMobileMenu} />
                ))}
              </div>
            </motion.div>
          </>
